...
The `ruby' backend talks to a ruby subprocess to do the handling.\"
"
  :type '(choice
...
(defun regex-render-ruby (regex sample)
  (with-temp-buffer
    (insert (format "DATA = <<-EOF
%s
EOF

$stdout.write \"(\"
DATA.scan(%%r{%s}m) do |i|
  m = $~
  $stdout.write \" (#{m.begin(0)} #{m.end(0) - m.begin(0)} \"
  (1..m.length - 1).each do |j|
    $stdout.write \" (#{j} . \\\"#{m[j]}\\\")\"
  end
  $stdout.write \")\"
end
    ...
    (when (> (length regex) 0)
      ...
      (with-current-buffer regex-text-buffer
        ...
        (save-excursion
          (ignore-errors
            (goto-char (point-min))
            (cond ((eq regex-tool-backend 'perl)
                  ...
                  ((eq regex-tool-backend 'ruby)
                   (let ((results (regex-render-ruby regex (buffer-string))))
                     (dolist (result results)
                       (let ((offset (nth 0 result))
                             (length (nth 1 result))
                             (matches (nthcdr 2 result)))
